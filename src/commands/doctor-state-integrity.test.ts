...
type EnvSnapshot = {
  HOME?: string;
  PROPAICLAW_HOME?: string;
  PROPAICLAW_STATE_DIR?: string;
  PROPAICLAW_OAUTH_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    PROPAICLAW_OAUTH_DIR: process.env.PROPAICLAW_OAUTH_DIR,
  };
}
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    process.env.PROPAICLAW_STATE_DIR = path.join(tempHome, ".propaiclaw");
    delete process.env.PROPAICLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.PROPAICLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when PROPAICLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.PROPAICLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
