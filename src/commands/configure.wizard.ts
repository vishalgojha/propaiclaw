...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? readGatewayTokenEnv(process.env);
  const password = params.cfg.gateway?.auth?.password ?? readGatewayPasswordEnv(process.env);
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? readGatewayTokenEnv(process.env),
      password: baseConfig.gateway?.auth?.password ?? readGatewayPasswordEnv(process.env),
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? envPassword;
    const oldPassword = baseConfig.gateway?.auth?.password ?? envPassword;
    const token = nextConfig.gateway?.auth?.token ?? readGatewayTokenEnv(process.env);
