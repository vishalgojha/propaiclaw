...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    PROPAICLAW_STATE_DIR: "/tmp/openclaw-daemon",
    PROPAICLAW_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.PROPAICLAW_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.PROPAICLAW_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.PROPAICLAW_STATE_DIR = "/tmp/openclaw-cli";
    process.env.PROPAICLAW_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
