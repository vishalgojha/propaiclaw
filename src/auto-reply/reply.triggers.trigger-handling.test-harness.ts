...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.PROPAICLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("PROPAICLAW_HOME", snapshot.openclawHome);
  restoreKey("PROPAICLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.PROPAICLAW_STATE_DIR = join(home, ".openclaw");
