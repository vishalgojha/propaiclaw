...
This directory contains hooks that ship with Propaiclaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.Propaiclaw/workspace`)

**Enable**:

```bash
Propaiclaw hooks enable session-memory
```
...
```bash
Propaiclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.Propaiclaw/logs/commands.log`

**Enable**:

```bash
Propaiclaw hooks enable command-logger
```
...
```bash
Propaiclaw hooks enable boot-md
```
...
homepage: https://docs.Propaiclaw.ai/automation/hooks#my-hook
metadata:
  { "Propaiclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.Propaiclaw/hooks/` (shared across workspaces)
...
```bash
Propaiclaw hooks list
```

Show hook details:

```bash
Propaiclaw hooks info session-memory
```

Check hook status:

```bash
Propaiclaw hooks check
```
...
```bash
Propaiclaw hooks enable session-memory
Propaiclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.Propaiclaw/Propaiclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'Propaiclaw.*gateway' && pnpm Propaiclaw gateway`
3. Enable the hook: `Propaiclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.Propaiclaw.ai/automation/hooks
