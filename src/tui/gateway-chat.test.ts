...
describe("resolveGatewayConnection", () => {
  ...
  it("uses PROPAICLAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ PROPAICLAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers PROPAICLAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ PROPAICLAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
