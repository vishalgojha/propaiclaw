...
function loadConfigForHome(home: string) {
  return createConfigIO({
    env: envWith({ PROPAICLAW_HOME: home }),
    homedir: () => home,
  }).loadConfig();
}
...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.openclaw when env not set", () => {
      expect(resolveStateDir(envWith({ PROPAICLAW_STATE_DIR: undefined }))).toMatch(/\.openclaw$/);
    });

    it("STATE_DIR respects PROPAICLAW_STATE_DIR override", () => {
      expect(resolveStateDir(envWith({ PROPAICLAW_STATE_DIR: "/custom/state/dir" }))).toBe(
        path.resolve("/custom/state/dir"),
      );
    });

    it("STATE_DIR respects PROPAICLAW_HOME when state override is unset", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveStateDir(envWith({ PROPAICLAW_HOME: customHome, PROPAICLAW_STATE_DIR: undefined })),
      ).toBe(path.join(path.resolve(customHome), ".openclaw"));
    });

    it("CONFIG_PATH defaults to PROPAICLAW_HOME/.openclaw/openclaw.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
            PROPAICLAW_HOME: customHome,
            PROPAICLAW_CONFIG_PATH: undefined,
    ...
    it("CONFIG_PATH defaults to ~/.openclaw/openclaw.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ PROPAICLAW_CONFIG_PATH: undefined, PROPAICLAW_STATE_DIR: undefined }),
        ),
      ).toMatch(/\.openclaw[\\/]openclaw\.json$/);
    });

    it("CONFIG_PATH respects PROPAICLAW_CONFIG_PATH override", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ PROPAICLAW_CONFIG_PATH: "/nix/store/abc/openclaw.json" }),
        ),
      ).toBe(path.resolve("/nix/store/abc/openclaw.json"));
    });

    it("CONFIG_PATH expands ~ in PROPAICLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ PROPAICLAW_HOME: home, PROPAICLAW_CONFIG_PATH: "~/.openclaw/custom.json" }),
            () => home,
          ),
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", () => {
      expect(resolveConfigPathCandidate(envWith({ PROPAICLAW_STATE_DIR: "/custom/state" }))).toBe(
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", () => {
      expect(resolveGatewayPort({}, envWith({ PROPAICLAW_GATEWAY_PORT: undefined }))).toBe(
        DEFAULT_GATEWAY_PORT,
      );
    });

    it("prefers PROPAICLAW_GATEWAY_PORT over config", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19002 } },
          envWith({ PROPAICLAW_GATEWAY_PORT: "19001" }),
        ),
      ).toBe(19001);
    });
    ...
    it("falls back to config when env is invalid", () => {
      expect(
        resolveGatewayPort(
          { gateway: { port: 19003 } },
          envWith({ PROPAICLAW_GATEWAY_PORT: "nope" }),
        ),
      ).toBe(19003);
    });
  });
