...
import { readGatewayPasswordEnv, readGatewayTokenEnv } from "./credentials.js";
...
async function setupGatewayTestHome() {
  ...
  process.env.PROPAICLAW_STATE_DIR = path.join(tempHome, ".openclaw");
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["PROPAICLAW_GATEWAY_TOKEN"]);
  const prev = readGatewayTokenEnv(process.env);
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PROPAICLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.PROPAICLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
