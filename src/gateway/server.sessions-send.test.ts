...
beforeAll(async () => {
  envSnapshot = captureEnv(["PROPAICLAW_GATEWAY_PORT", "PROPAICLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.PROPAICLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.PROPAICLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("PROPAICLAW_CONFIG_PATH missing in gateway test environment");
      }
