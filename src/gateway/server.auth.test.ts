...
function resolveGatewayTokenOrEnv(): string {
  const token =
    typeof (testState.gatewayAuth as { token?: unknown } | undefined)?.token === "string"
      ...
      : process.env.PROPAICLAW_GATEWAY_TOKEN;
...
describe("gateway server auth/connect", () => {
      ...
      for (const testCase of [
        {
          env: {
            PROPAICLAW_VERSION: " ",
            PROPAICLAW_SERVICE_VERSION: "2.4.6-service",
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.PROPAICLAW_GATEWAY_TOKEN;
      process.env.PROPAICLAW_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
