...
Propaiclaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
Propaiclaw channels login --channel zalouser
...
```bash
Propaiclaw message send --channel zalouser --target <threadId> --message "Hello from Propaiclaw!"
```
...
```bash
Propaiclaw channels login --channel zalouser              # Login via QR
Propaiclaw channels login --channel zalouser --account work
Propaiclaw channels status --probe
Propaiclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
Propaiclaw directory self --channel zalouser
Propaiclaw directory peers list --channel zalouser --query "name"
Propaiclaw directory groups list --channel zalouser --query "work"
Propaiclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
Propaiclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
Propaiclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `Propaiclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
Propaiclaw channels login --channel zalouser --account work
Propaiclaw message send --channel zalouser --account work --target <id> --message "Hello"
