# Google Gemini CLI Auth (Propaiclaw plugin)
...
```bash
Propaiclaw plugins enable google-gemini-cli-auth
```
...
```bash
Propaiclaw models auth login --provider google-gemini-cli --set-default
```
...
- `PROPAICLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `PROPAICLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
