...
`Propaiclaw.invoke --each`) without adding new Propaiclaw code per workflow.
...
This extension depends on Propaiclaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Propaiclaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.Propaiclaw/extensions` as a standalone plugin directory.
